...
-- Tables are fully qualified (HQ.PUBLIC.x); run each query with the cursor inside it (Ctrl+Enter).
...
FROM ranked r
JOIN HQ.PUBLIC.hospitals h USING (facility_id)
...
w AS (
    ...
    FROM latest l JOIN HQ.PUBLIC.hospitals h USING (facility_id)
),
d AS (SELECT measure_id, MAX(IFF(higher_is_better, 1, 0)) = 1 AS hib FROM HQ.PUBLIC.measures GROUP BY measure_id)
SELECT w.measure_id,
...
WITH tx AS (SELECT COUNT(*) AS n FROM HQ.PUBLIC.hospitals WHERE is_texas),
cov AS (
    SELECT mv.measure_id,
           COUNT(DISTINCT IFF(h.is_texas AND mv.score IS NOT NULL, mv.facility_id, NULL)) AS tx_reporting
    FROM HQ.PUBLIC.measure_values mv JOIN HQ.PUBLIC.hospitals h USING (facility_id)
