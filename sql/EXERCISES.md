...
**01_houston_readmit_scorecard** — For every Houston-area hospital, show name, county, ownership, overall star rating, and its pneumonia 30-day readmission score (`READM_30_PN`, latest period). Include hospitals with no score (LEFT JOIN). Sort by score ascending.
...
**05_vs_state_average** — For every Texas hospital with a `READM_30_PN` score, show its score, the Texas average (`AVG(score) OVER ()`), and the difference. Then add the *county* average as a second window column. Sort by difference (most below average first).
...
**08_quality_composite** — Build a composite score for Texas hospitals from three domains: z-score each of `READM_30_PN` (readmission, flip sign so higher = better), `H_RECMND_DY` (recommend), and `SEP_1` (sepsis care) across Texas, then average the three z-scores per hospital. Use one CTE per domain + a final CTE that joins them. Return the top 10 and bottom 10 with all three z's shown.
...
**09_mismatch_finder** — Find Texas hospitals that are *better than the state average* on readmission (`READM_30_PN` below TX mean) but *worse than average* on patient experience (`H_RECMND_DY` below TX mean). Use CTEs for the two averages. Return hospital, county, both scores, both averages.
*Analyst task: the interesting quadrant — clinically good, experientially weak. Great dashboard annotation material.*

**10_county_rollup** — County-level roll-up for Texas: hospital count, count with a `READM_30_PN` score, mean readmission score, and a *denominator-weighted* mean readmission (`SUM(score*denominator)/SUM(denominator)`). CTE for per-hospital latest values, then aggregate. Sort by weighted mean. Why do the two means differ?

## Tier 4 — aggregation & pivot (2 exercises)

**11_scorecard_pivot** — One row per Houston-area hospital with columns: `readm_pn`, `mort_hf`, `hai_clabsi_sir`, `sepsis_pct`, `recommend_pct`, `star_rating` — pulled from the long table with `MAX(score) FILTER (WHERE measure_id = '...')` (or CASE WHEN). Sort by recommend_pct desc. This is the dashboard extract shape.
